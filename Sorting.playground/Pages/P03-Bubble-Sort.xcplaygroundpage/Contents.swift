...
/*:
 ...
 <!-- TODO: I don't think this makes sense. We assume they make a very specific mistake. I don't think that all, or even necessarily a majority of students will make the mistake of iterating over it only once. We need to help them with some sort of test. E.g. run the deck instantiation, then use the sort, then see if deck.isSorted == true. If so, "Congrats!!", else "Why might that be? Let's take a closer look. A common mistake is for students to only iterate through the array once...."  --!>
 ...
 <!-- TODO: Needs better explanation. The set is so small that it's an instantaneous sort, plus they have no basis for efficiency anyway. At the very least, we could leave this here, then at the very end talk about run time complexity using the examples they just coded as their foundation. --!>
 Perhaps you're not satisfied. Maybe this sorting algorithm didn't seem very efficient to you. It sort of defies common sense. When you are sorting cards by hand you probably move the largest card to the end of the hand and then repeat the process. You likely _scan_ through the pile looking for the largest value, instead of comparing each card against its neighbor and doing this bubbling process. We'll explore this room for improvement in the next section and introduce a faster sorting algorithm called Selection Sort.
